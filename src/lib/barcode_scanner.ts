...
export class BarcodeScanner {
  ...
  async start(
    ...
    try {
      ...
      this.zxingReader.decodeFromVideoElement(video, (result: any, error: any, controls: any) => {
        ...
        if (error && onScanFailure) {
          // ZXing melempar error frame-by-frame jika tidak mendeteksi kode, kita abaikan saja agar tidak bising
        }
      });
  ...
  /**
   * Scan barcode dari file gambar yang di-upload
   */
  async scanFile(
    file: File,
    onScanSuccess: (decodedText: string) => void,
    ...
    // Pilihan A: Native BarcodeDetector (ML Kit) dengan fallback ke ZXing jika gagal mendeteksi
    if (BarcodeScanner.isNativeSupported()) {
      try {
        const detector = new (window as any).BarcodeDetector({ formats: nativeFormats });
        const img = new Image();
        img.onload = async () => {
          try {
            ...
            if (barcodes.length > 0) {
              ...
              this.scanFileWithZxingFallback(file, onScanSuccess, onScanFailure);
            }
          } catch (err: any) {
            this.scanFileWithZxingFallback(file, onScanSuccess, onScanFailure);
          } finally {
        ...
        console.warn('Native BarcodeDetector file scan error, menggunakan ZXing:', err);
      }
    }

    // Pilihan B: Fallback langsung ke ZXing
    this.scanFileWithZxingFallback(file, onScanSuccess, onScanFailure);
  }

  private scanFileWithZxingFallback(
    file: File,
    onScanSuccess: (decodedText: string) => void,
    onScanFailure: (error: string) => void
  ) {
    ...
    img.onload = async () => {
      try {
        ...
        if (result) {
          ...
          onScanFailure('Barcode tidak ditemukan dalam gambar.');
        }
      } catch (err: any) {
        console.warn('ZXing file scan gagal:', err);
        onScanFailure(err?.message || 'Barcode tidak ditemukan dalam gambar.');
      } finally {
        URL.revokeObjectURL(img.src);
      }
    };
    img.onerror = () => {
      onScanFailure('Gagal memuat gambar barcode.');
      URL.revokeObjectURL(img.src);
    };
    img.src = URL.createObjectURL(file);
  }
  ...
  async stop() {
    try {
      ...
      // Bersihkan viewport container DOM
